...
public class CommandBlockLight extends CommandBase implements ICommand {

	private final List<String> aliases = new ArrayList<String>();

	public CommandBlockLight() {
		aliases.add("blocklight");
	}
	...
	public String getCommandUsage(ICommandSender sender) {
		return "commands.blocklight.usage";
	}
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length < 4) {
			throw new WrongUsageException("commands.blocklight.usage", new Object[0]);
		}
		...
		int lightLevel = sender.getEntityWorld().getLightFor(EnumSkyBlock.BLOCK, pos);
		if ("test".equalsIgnoreCase(args[0])) {
			if (args.length < 5 || args.length > 6) {
				throw new WrongUsageException("commands.blocklight.test.usage", new Object[0]);
			}
			...
			if (min > lightLevel || max < lightLevel) {
				throw new CommandException("commands.blocklight.test.failed", new Object[] { x.getResult(),
						y.getResult(), z.getResult(), lightLevel, min, max });
			}
			else {
				notifyCommandListener(sender, this, "commands.blocklight.test.success", new Object[] { x.getResult(),
						y.getResult(), z.getResult(), lightLevel, min, max });
			}
			...
			notifyCommandListener(sender, this, "commands.blocklight.query", new Object[] { x.getResult(),
					y.getResult(), z.getResult(), lightLevel });
		}
	}
