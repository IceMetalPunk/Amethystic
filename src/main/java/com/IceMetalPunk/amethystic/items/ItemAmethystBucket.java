...
public class ItemAmethystBucket extends Item {
	...
	public ItemAmethystBucket(Block containedBlock, String resourceLocation) {
		...
		this.setUnlocalizedName(resourceLocation).setRegistryName(Amethystic.MODID, resourceLocation)
				...
				if (!playerIn.canPlayerEdit(blockpos.offset(raytraceresult.sideHit), raytraceresult.sideHit,
					...
					if (material == Material.WATER
						...
						return new ActionResult(EnumActionResult.SUCCESS,
								this.fillBucket(itemStackIn, playerIn, Amethystic.items.AMETHYST_WATER_BUCKET));
						...
						playerIn.addStat(StatList.getObjectUseStats(this));
						return new ActionResult(EnumActionResult.SUCCESS,
								this.fillBucket(itemStackIn, playerIn, Amethystic.items.AMETHYST_LAVA_BUCKET));
					} else {
						return new ActionResult(EnumActionResult.FAIL, itemStackIn);
					}
				}
				...
				if (!playerIn.canPlayerEdit(blockpos1, raytraceresult.sideHit, itemStackIn)) {
					...
					return !playerIn.capabilities.isCreativeMode
							? new ActionResult(EnumActionResult.SUCCESS,
									new ItemStack(Amethystic.items.AMETHYST_BUCKET))
